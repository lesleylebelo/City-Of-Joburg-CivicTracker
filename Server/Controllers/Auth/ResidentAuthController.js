...
const generateToken = require("../../Utils/GenerateToken"); // Ensure local file is named GenerateToken.js
...
const registerResident = async (req, res) => {
    ...
    try {
        ...
        const emailCheck = await pool.request()
            ...
            .query(`
                SELECT resident_id FROM residents WHERE email = @email
            `);
...
const loginResident = async (req, res) => {
    ...
    try {
        ...
        // Explicit structural fallback check for account state deactivation
        if (resident.is_active === false || resident.is_active === 0) {
        ...
        // Generate tracking token payload matching authorization expectations
